...
## 项目概述
这个仓库在标准 SPDK 之上进行深度定制，核心目标包括：精简默认组件、强化 RAID/EC 功能、提供磨损均衡与 SMART 感知、以及扩展 Go 侧的外部通知能力。为方便追溯，本 README 逐条记录截至 `ee99cbd` 的全部 56 次提交（包括体量很小的修复）。

## 关键里程碑速览
- **基础环境**：清理上游文档/测试、裁剪 GitHub Actions、重写 Version 体系并重新管理多个子模块，确保仓库可在离线环境快速拉起。
...
- **测试与文档**：虽然主线移除了大规模上游测试，但补充了聚焦 RAID/EC 的专用单测、使用指南及 demo，保证新特性可验证、可学习。
- **Go 能力**：新增 `go/notifybridge` 及 C 入口，支持在 NVMe 设备移除时触发 Go 回调，方便与外部管控系统联动。
...
4. **2025-10-16 · e89c7f2 – 修改spdk输出为mimo品牌标识**：在 `spdk_tgt`、env DPDK 初始化及 reactor 中替换品牌信息，统一日志输出形象。
...
17. **2025-10-30 · 813b291 – 修改Makefile的编译程序命名**：微调 `app/spdk_tgt/Makefile`，统一编译生成的程序名。
...
55. **2025-11-19 · c3f7f2d – 实现了 Go 调用仓库**：新增 `go/notifybridge`（含 README、示例配置、模块依赖）及 C 头文件/入口，把 Go 进程桥接到 `spdk_tgt`。
56. **2025-11-19 · ee99cbd – Add Go notify hook for NVMe removal**：把 Go 钩子整合进通用 `mk/spdk.app.mk`，并在 `bdev_nvme.c` 中触发 NVMe 删除事件通知，同时精简 `spdk_tgt` Makefile。
